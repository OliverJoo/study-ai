    ...
    while pq:
        ...
        for dy, dx in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if 0 <= ny < rows and 0 <= nx < cols and grid[ny][nx] == 0:
                new_g_cost = g_cost + 1
                heapq.heappush(
                    pq, (new_g_cost + heuristic((ny, nx), end),
...
class OptimizedPathfinder:
    ...
    def get_cache_stats(self):
        ...
        return {
            'distance_cache': dist_stats,
            'path_cache': path_stats
        }
...
class ProgressMonitor:
    ...
    def update(self, increment=1):
        ...
        if self.current_task > 0 and self.current_task < self.total_tasks:
            ...
            print(f"\r{self.task_name} Progress: {progress_percent:.1f}% "
                  f"({self.current_task:,}/{self.total_tasks:,}) | "
                  f"Elapsed: {elapsed_time:.1f}s | "
                  f"ETA: {estimated_remaining_time:.1f}s", end="")
        else:
            print(f"\r{self.task_name} Progress: {progress_percent:.1f}% "
                  f"({self.current_task:,}/{self.total_tasks:,})", end="")
...
class CompleteTSPSolver:
    ...
    def solve_complete_tsp(self, waypoints_dict, start_key, end_key):
        ...
        if start_key not in waypoints_dict or end_key not in waypoints_dict:
            print(f"Error: Start key ({start_key}) or end key ({end_key}) not found")
            return [], float('inf')
        
        # Extract ALL intermediate waypoints (exclude start and end)
        intermediate_keys = [key for key in waypoints_dict.keys() 
        ...
        if total_permutations > 100000:
            print("  Warning: High computational complexity. Using optimized approach...")
            return self._optimized_large_tsp(waypoints_dict, start_key, end_key, intermediate_keys)
        
        # Complete TSP for smaller problems
        return self._complete_small_tsp(waypoints_dict, start_key, end_key, intermediate_keys)
    
    def _complete_small_tsp(self, waypoints_dict, start_key, end_key, intermediate_keys):
        ...
        for permutation in itertools.permutations(intermediate_keys):
            # Construct complete sequence: start -> all intermediates -> end
            current_sequence = [start_key] + list(permutation) + [end_key]
            current_cost = self._calculate_sequence_cost(current_sequence, waypoints_dict)
            
            if current_cost < best_cost:
                best_cost = current_cost
                best_sequence = current_sequence
                
                # Construct full path for best sequence
    ...
    def _optimized_large_tsp(self, waypoints_dict, start_key, end_key, intermediate_keys):
        """Optimized approach for large TSP problems using clustering"""
        # Use clustering to reduce problem size
        clusters = self._cluster_waypoints(waypoints_dict, intermediate_keys)
        print(f"Clustering result: {len(clusters)} clusters")
        
        # Solve cluster order
        cluster_order = self._optimize_cluster_order(clusters, waypoints_dict, start_key)
        ...
        for cluster_idx in cluster_order:
            ...
            if cluster_keys:
                # Find optimal entry point to cluster
                entry_key = min(cluster_keys, 
                              key=lambda k: self.pathfinder.get_distance(current_pos, waypoints_dict[k]))
                
                # Solve small TSP within cluster
                cluster_sequence, cluster_cost = self._solve_cluster_tsp(cluster_keys, entry_key)
    ...
    def _solve_cluster_tsp(self, cluster_keys, entry_key):
        ...
        # Try all permutations within cluster
        for permutation in itertools.permutations(other_keys):
            current_path = [entry_key] + list(permutation)
            current_cost = self._calculate_sequence_cost(current_path, waypoints)
        ...
        for i in range(len(sequence) - 1):
            ...
            if start_key in waypoints_dict and end_key in waypoints_dict:
                segment_path = self.pathfinder.get_path(
                    waypoints_dict[start_key], 
                    waypoints_dict[end_key]
                )
                if segment_path:
                    ...
                    if i == 0:
                        ...
                        full_path.extend(segment_list[1:])  # Remove duplicate point
...
def draw_map_with_path(df, path, output_filename, title="Shortest Path Map"):
    ...
    try:
        ...
        if path and len(path) > 1:
            ...
            ax.plot(path_x[0], path_y[0], marker="o", color="lime", 
                   markersize=15, markeredgecolor="darkgreen", markeredgewidth=2)
            ax.plot(path_x[-1], path_y[-1], marker="o", color="gold", 
                   markersize=15, markeredgecolor="darkorange", markeredgewidth=2)
        
        legend_elements = [
            patches.Patch(facecolor="gray", edgecolor="black", label="Construction Site (Blocked)"),
            plt.Line2D([0], [0], marker="o", color="w", label="Apartment",
                      markerfacecolor="brown", markersize=12),
            plt.Line2D([0], [0], marker="s", color="w", label="Building",
                      markerfacecolor="darkblue", markersize=12),
            plt.Line2D([0], [0], marker="^", color="w", label="Bandalgom Coffee",
                      markerfacecolor="green", markersize=15),
            plt.Line2D([0], [0], marker="*", color="w", label="My Home",
...
def main():
    ...
    try:
        ...
        for _, row in construction_sites.iterrows():
            grid[row["y"]][row["x"]] = 1
        print(f"Construction sites set as obstacles: {len(construction_sites)} locations")

        # Find start and destination points
        my_home = df[df["struct"].str.strip().str.replace("_", "").str.lower() == "myhome"]
        cafes = df[df["struct"].str.strip().str.replace("_", "").str.lower() == "bandalgomcoffee"]
        ...
        # Create result_img directory
        result_img_dir = os.path.join(base_dir, "result_img")
        os.makedirs(result_img_dir, exist_ok=True)

        # Main task: Shortest path from home to nearest cafe
        ...
        if shortest_path:
            ...
            # *** 추가된 부분: home_to_cafe.csv 파일로 경로 저장 ***
            ...
            draw_map_with_path(df, shortest_path, map_final_path, 
                             "Home to Nearest Cafe Shortest Path")
        else:
            print("No path found to any cafe.")

        # Bonus task: Complete TSP visiting ALL structures
        print("\n=== Bonus Task: Complete Structure Visitation TSP ===")
        
        pathfinder = OptimizedPathfinder(grid)
        
        # Build complete waypoints dictionary for ALL structures
        all_structures = df[df["ConstructionSite"] == 0].copy()
        global waypoints
        waypoints = {}
        
        structure_counts = {"apartment": 0, "building": 0, "bandalgomcoffee": 0, "myhome": 0}
        ...
        if not bandalgom_start_key or not my_home_end_key:
            ...
            # Execute complete TSP
            complete_solver = CompleteTSPSolver(pathfinder)
            
            tsp_start_time = time.time()
            optimal_sequence, optimal_full_path, optimal_cost = complete_solver.solve_complete_tsp(
            ...
            if optimal_sequence and optimal_cost != float('inf'):
                readable_path = " -> ".join([key.split("_")[0].title() for key in optimal_sequence])
                print(f"Visit sequence: {readable_path}")
                print(f"Total waypoints: {len(optimal_sequence)}")
                
                # Verify complete visitation
                ...
                print(f"Complete visitation: {'YES' if len(visited_structures) >= 4 else 'NO'}")
                
                # Save bonus result
                if optimal_full_path:
                    map_final_bonus_path = os.path.join(result_img_dir, "map_final_bonus.png")
                    draw_map_with_path(df, optimal_full_path, map_final_bonus_path, 
        ...
        if 'pathfinder' in locals():
            ...
            if total_dist_calls > 0:
                print(f"  Distance calculation cache efficiency: {dist_cache.hits/total_dist_calls*100:.1f}%")
            if total_path_calls > 0:
                print(f"  Path calculation cache efficiency: {path_cache.hits/total_path_calls*100:.1f}%")
